...
// ─────────────────────────────────────────────
//  AI MEASUREMENT USAGE (Pro Monthly cap / Pro Yearly unlimited)
// ─────────────────────────────────────────────
const AI_USAGE_MONTHLY_LIMIT = 25; // Pro Monthly cap (Pro Yearly + trial/free are unlimited)
...
// Returns { unlimited, used, limit, remaining } for the current boutique without incrementing
app.get('/api/ai-measurements/usage', auth, async (req, res) => {
  try {
    const result = await db.query(
      'SELECT plan, is_free, ai_usage_count, ai_usage_month, expires_at FROM boutiques WHERE id = $1',
      [req.boutiqueId]
    );
    if (!result.rows.length) return res.status(404).json({ error: 'Not found' });
    const b = result.rows[0];

    const isTrialActive = b.plan === 'trial' && b.expires_at && new Date(b.expires_at) > new Date();
    const unlimited = b.plan === 'pro' || b.is_free === true || isTrialActive;

    const month = currentMonthKey();
    const used = (b.ai_usage_month === month) ? b.ai_usage_count : 0;
    const limit = b.plan === 'pro_monthly' ? AI_USAGE_MONTHLY_LIMIT : (unlimited ? null : 0);
...
// Records one AI measurement suggestion use; enforces the Pro Monthly cap.
// Pro Yearly, free, and active-trial accounts are unlimited (not counted against a cap).
app.post('/api/ai-measurements/usage', auth, async (req, res) => {
  try {
    const result = await db.query(
      'SELECT plan, is_free, ai_usage_count, ai_usage_month, expires_at FROM boutiques WHERE id = $1',
      [req.boutiqueId]
    ...
    if (unlimited) {
      return res.json({ unlimited: true, used: null, limit: null, remaining: null });
    }

    if (b.plan !== 'pro_monthly') {
      // Basic plans don't have access to AI measurements at all
      return res.status(403).json({ error: 'AI Measurement Suggestions are a Pro feature. Please upgrade to Pro.' });
    }

    const month = currentMonthKey();
    const used = (b.ai_usage_month === month) ? b.ai_usage_count : 0;

    if (used >= AI_USAGE_MONTHLY_LIMIT) {
      return res.status(403).json({
        error: `You've used all ${AI_USAGE_MONTHLY_LIMIT} AI Measurement Suggestions for this month. Upgrade to Pro Yearly for unlimited access.`,
        unlimited: false,
        used,
    ...
    await db.query(
      'UPDATE boutiques SET ai_usage_count = $1, ai_usage_month = $2, updated_at = NOW() WHERE id = $3',
      [newCount, month, req.boutiqueId]
    );
    ...
    res.json({
      ...
      used: newCount,
      limit: AI_USAGE_MONTHLY_LIMIT,
      remaining: Math.max(0, AI_USAGE_MONTHLY_LIMIT - newCount),
    });
  } catch (e) {
    console.error(e);
    res.status(500).json({ error: 'Server error' });
...
app.get('/api/auth/me', auth, async (req, res) => {
  try {
    const result = await db.query(
      `SELECT id, name, owner_name, email, phone, city, address, gstin, logo_url, plan, is_free,
              expires_at, ai_usage_count, ai_usage_month, created_at
  ...
  <div class="modal" id="modal">
    ...
    <div class="modal-actions">
      ...
      <button class="action-btn ab-plan" onclick="togglePlanRow()">
        ...
        <div class="ab-text">Change Plan<span>Switch between Free / Monthly / Yearly / Pro Monthly / Pro Yearly</span></div>
      </button>
      <div id="plan-row" style="display:none;margin-top:-4px;background:#f0f7ff;border-radius:10px;padding:14px;border:1px solid #bfdbfe;">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:10px;">
          ...
          <button onclick="doChangePlan('pro_monthly')" style="padding:8px 14px;border-radius:7px;border:1.5px solid #d4a574;background:#fff;color:#b8895a;font-size:12px;font-weight:700;cursor:pointer;letter-spacing:0.5px;">PRO MONTHLY <span style="font-size:10px;opacity:0.7">(25 AI/mo)</span></button>
          <button onclick="doChangePlan('pro')"         style="padding:8px 14px;border-radius:7px;border:1.5px solid #d4a574;background:#1a1a2e;color:#d4a574;font-size:12px;font-weight:700;cursor:pointer;letter-spacing:0.5px;">PRO YEARLY <span style="font-size:10px;opacity:0.7">(Unlimited AI)</span></button>
        </div>
...
function expiryInfo(b) {
  const planNames = { pro: 'Pro Yearly', pro_monthly: 'Pro Monthly' };
  const planName = planNames[b.plan] || ((b.plan || 'free').charAt(0).toUpperCase() + (b.plan || 'free').slice(1));
    ...
    return (
      '<div class="boutique-card ' + cardCls + '" onclick="openModal(' + b.id + ')">' +
        ...
        '<div class="info">' +
          ...
          '<div class="tags">' + statusTag + '<span class="tag t-plan">' + (b.plan || 'free').toUpperCase().replace('_', ' ') + '</span>' + (b.city ? '<span class="tag t-city">' + b.city + '</span>' : '') + '</div>' +
          expiryRowHtml +
          loginHtml +
          lastActHtml +
        '</div>' +
...
function openModal(id) {
  ...
  document.getElementById('m-details').innerHTML =
    ...
    '<div class="detail-item"><div class="dl">PLAN</div><div class="dv">' + (b.plan || 'free').toUpperCase().replace('_', ' ') + '</div></div>' +
...
app.patch('/api/admin/boutiques/:id/renew', adminAuth, async (req, res) => {
  try {
    ...
    const curPlan = cur.rows[0]?.plan;
    const plan = months >= 12
      ? (curPlan === 'pro' ? 'pro' : 'yearly')
      : (curPlan === 'pro_monthly' ? 'pro_monthly' : 'monthly');
...
app.patch('/api/admin/boutiques/:id/plan', adminAuth, async (req, res) => {
  try {
    ...
    // 'pro' = PRO YEARLY (unlimited AI measurements), 'pro_monthly' = PRO MONTHLY (capped AI measurements)
    const allowed = ['free', 'monthly', 'yearly', 'pro', 'pro_monthly'];
    if (!allowed.includes(plan)) return res.status(400).json({ error: 'Invalid plan' });

    let result;
    if (plan === 'pro') {
      // PRO (yearly) is always yearly — auto-extend expiry to 1 year from today (or from existing expiry if later)
      ...
      // PRO MONTHLY — auto-extend expiry to 1 month from today (or from existing expiry if later)
      result = await db.query(
        `UPDATE boutiques
         SET plan=$1, is_active=true,
             expires_at = GREATEST(NOW(), COALESCE(expires_at, NOW())) + INTERVAL '1 month',
             updated_at = NOW()
         WHERE id=$2
         RETURNING id, name, plan, expires_at`,
        [plan, id]
    ...
    if (!result.rows.length) return res.status(404).json({ error: 'Boutique not found' });
    await logAction(id, 'Plan Changed', `→ ${plan.toUpperCase()}` + (plan === 'pro' ? ' (Yearly)' : plan === 'pro_monthly' ? ' (Monthly)' : '') + (reason ? ` · ${reason}` : ''));
